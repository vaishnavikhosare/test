...
// Limit handling
$limit = 20;
if (isset($_SESSION['limit'])) $limit = intval($_SESSION['limit']);
if (isset($_GET['limit'])) {
...
// Total records
$totalQuery = "SELECT COUNT(*) AS total FROM builders";
...
<h4 class="mb-4">Builders List</h4>
...
<table class="table">
    ...
    <tbody id="TableBody">
        ...
        <tr>
            ...
            <td>
                <div>
                    <strong><?= htmlspecialchars($row['companyName']) ?></strong><br>
                   
                    Established: <?= htmlspecialchars($row['establishedYear']) ?><br>
                    <a href="mailto:<?= htmlspecialchars($row['publicEmail']) ?>"><?= htmlspecialchars($row['publicEmail']) ?></a><br>
                    <a href="tel:<?= htmlspecialchars($row['publicContact']) ?>"><?= htmlspecialchars($row['publicContact']) ?></a><br>
                    <?php if ($row['website']): ?>
            ...
            <td>
                <div>
                    <strong><a href="user-details.php?userID=<?= $row['userID'] ?>&type=<?= $row['userType'] ?>" target="_blank"><?= htmlspecialchars($row['username']) ?></a></strong><br>
    ...
    <nav>
        <ul class="pagination justify-content-center">
            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                <a class="btn btn-common" href="?builder&page=<?= max(1, $page - 1) ?>"><i class="lni-chevron-left"></i> Previous</a>
            </li>
            ...
            ?>
                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                    <a class="page-link" href="?builder&page=<?= $i ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>

            <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>">
                <a class="btn btn-common" href="?builder&page=<?= min($totalPages, $page + 1) ?>">Next <i class="lni-chevron-right"></i></a>
            </li>
        </ul>
    </nav>
</div></div></div>
